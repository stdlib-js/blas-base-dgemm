...
var rntantb = require( './fixtures/row_major_nta_ntb.json' ); // eslint-disable-line stdlib/no-empty-lines-between-requires
...
tape( 'the function has an arity of 14', function test( t ) {
	t.strictEqual( dgemm.length, 14, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided an invalid first argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( value, data.transA, data.transB, data.M, data.N, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid second argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, value, data.transB, data.M, data.N, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid third argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, value, data.M, data.N, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fourth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, data.transB, value, data.N, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fifth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, data.transB, data.M, value, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid sixth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, data.transB, data.M, data.N, value, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid ninth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, data.transB, data.M, data.N, data.K, data.alpha, new Float64Array( data.A ), value, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid eleventh argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, data.transB, data.M, data.N, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), value, data.beta, new Float64Array( data.C ), data.ldc );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fourteenth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dgemm( data.order, data.transA, data.transB, data.M, data.N, data.K, data.alpha, new Float64Array( data.A ), data.lda, new Float64Array( data.B ), data.ldb, data.beta, new Float64Array( data.C ), value );
		};
	}
});
...
tape( 'if either `M` or `N` is `0`, the function returns the third input matrix unchanged (row-major)', function test( t ) {
	...
	out = dgemm( data.order, data.transA, data.transB, 0, data.N, data.K, data.alpha, a, data.lda, b, data.ldb, data.beta, c, data.ldc );
	t.strictEqual( out, c, 'returns expected value' );
	t.deepEqual( out, expected, 'returns expected value' );

	out = dgemm( data.order, data.transA, data.transB, data.M, 0, data.K, data.alpha, a, data.lda, b, data.ldb, data.beta, c, data.ldc );
	t.strictEqual( out, c, 'returns expected value' );
	t.deepEqual( out, expected, 'returns expected value' );
...
tape( 'if `α` or `K` is `0` and `β` is `1`, the function returns the third input matrix unchanged (row-major)', function test( t ) {
	...
	out = dgemm( data.order, data.transA, data.transB, data.M, data.N, data.K, 0.0, a, data.lda, b, data.ldb, 1.0, c, data.ldc );
	t.strictEqual( out, c, 'returns expected value' );
	t.deepEqual( out, expected, 'returns expected value' );

	out = dgemm( data.order, data.transA, data.transB, data.M, data.N, 0, data.alpha, a, data.lda, b, data.ldb, 1.0, c, data.ldc );
	t.strictEqual( out, c, 'returns expected value' );
	t.deepEqual( out, expected, 'returns expected value' );
...
tape( 'if `α` is `0` and `β` is `0`, the function returns the third input matrix filled with zeros (row-major)', function test( t ) {
	...
	out = dgemm( data.order, data.transA, data.transB, data.M, data.N, data.K, 0.0, a, data.lda, b, data.ldb, 0.0, c, data.ldc );
	t.strictEqual( out, c, 'returns expected value' );
	t.deepEqual( out, expected, 'returns expected value' );
...
tape( 'if `α` is `0` and `β` is neither `0` nor `1`, the function returns the third input matrix scaled by `β` (row-major)', function test( t ) {
	...
	out = dgemm( data.order, data.transA, data.transB, data.M, data.N, data.K, 0.0, a, data.lda, b, data.ldb, 10.0, c, data.ldc );
	t.strictEqual( out, c, 'returns expected value' );
	t.deepEqual( out, expected, 'returns expected value' );
